...
const { listContacts, getBuId, addContact, removeContact, updateContact, updateStatusContact, listFilteredContacts } = require('../../models/contacts');
...
router.get('/:contactId', authenticate, isValidId, async (req, res, next) => {
  try {
    const contact = await getBuId(req.params.contactId);
    if (contact) {
